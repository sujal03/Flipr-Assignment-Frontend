document.addEventListener("DOMContentLoaded", function () {
    ...
    function fetchPrizes() {
      return Promise.resolve(jsonData.prizes);
    }
    ...
    function displayWinners(prizes) {
      ...
      prizes.forEach(prize => {
        ...
        listItem.className = 'nobel-item';
        listItem.innerHTML = `
          <strong>${prize.year}</strong> - ${prize.category}<br>
          ${prize.laureates.map(laureate => `${laureate.firstname} ${laureate.surname}`).join(', ')}<br>
          <em>${prize.motivation}</em>
        `;
    ...
    function displayMultipleWinners(prizes) {
      ...
      prizes.forEach(prize => {
        prize.laureates.forEach(laureate => {
          ...
          if (multipleTimeWinners[key]) {
            multipleTimeWinners[key]++;
          } else {
            multipleTimeWinners[key] = 1;
          }
        });
      });
  
      Object.keys(multipleTimeWinners).forEach(key => {
        if (multipleTimeWinners[key] > 1) {
          const listItem = document.createElement('li');
          listItem.className = 'nobel-item';
          listItem.textContent = `${key} - ${multipleTimeWinners[key]} times`;
          multipleWinners.appendChild(listItem);
        }
      });
    }
    ...
    async function init() {
      ...
      populateDropdown(prizes, 'category');
      populateDropdown(prizes, 'year');
      ...
      filterForm.addEventListener('submit', async function (event) {
        ...
        const filteredPrizes = prizes.filter(prize => {
          return (!selectedCategory || prize.category === selectedCategory) &&
